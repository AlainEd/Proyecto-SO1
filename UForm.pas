...
begin
  ...
  begin
    ...
    //colision: puede chocar con la parte de arriba o izq del usuario
      if colisionNaveUser(PRUN, USER) then   //choca der
      begin
    ...
    if Turno2 = 'DER' then
    begin
      if PRUN.Limite < 60 then
      begin
        PRUN.x := PRUN.x + 3;
        PRUN.Limite := PRUN.Limite + 3;
      end
      else
        Turno2 := 'IZQ';
    end;
...
begin
   if (PRUNBALA.x >= PRUNNAVE.x) and (PRUNBALA.x < PRUNNAVE.x + PRUNNAVE.Ancho - 5) and
      (PRUNBALA.y = PRUNNAVE.y + PRUNNAVE.Alto - 5) or

      ((PRUNBALA.x + PRUNBALA.Ancho = PRUNNAVE.x) and
      ((PRUNBALA.y >= PRUNNAVE.y) and (PRUNBALA.y <= PRUNNAVE.y + PRUNNAVE.Alto))) or

      ((PRUNBALA.x = PRUNNAVE.x + PRUNNAVE.Ancho)) and
      ((PRUNBALA.y >= PRUNNAVE.y) and (PRUNBALA.y <= PRUNNAVE.y + PRUNNAVE.Alto)) then
      Result := true
   else
      Result:= false;
end;
...
function TForm1.colisionNaveUser(PRUNNAVE, PRUNUSER: PCB):bool;
begin
   if (PRUNNAVE.x + PRUNNAVE.Ancho = PRUNUSER.x) and
       ((PRUNNAVE.y >= PRUNUSER.y) and (PRUNNAVE.y <= PRUNUSER.y + PRUNUSER.Alto) or
       (PRUNNAVE.y + PRUNNAVE.Alto >= PRUNUSER.y) and (PRUNNAVE.y + PRUNNAVE.Alto <= PRUNUSER.y + PRUNUSER.Alto)) or  //choca izq

       (PRUNNAVE.y + PRUNNAVE.Alto = PRUNUSER.y - 1) and
       ((PRUNNAVE.x >= PRUNUSER.x) and (PRUNNAVE.x <= PRUNUSER.x + PRUNUSER.Ancho) or
       (PRUNNAVE.x + PRUNNAVE.Ancho >= PRUNUSER.x) and (PRUNNAVE.x + PRUNNAVE.Ancho <= PRUNUSER.x + PRUNUSER.Ancho)) or  //choca arriba

       (PRUNNAVE.x -2 = PRUNUSER.x + PRUNUSER.Ancho) and
       ((PRUNNAVE.y >= PRUNUSER.y) and (PRUNNAVE.y <= PRUNUSER.y + PRUNUSER.Alto) or
       (PRUNNAVE.y + PRUNNAVE.Alto >= PRUNUSER.y) and (PRUNNAVE.y + PRUNNAVE.Alto <= PRUNUSER.y + PRUNUSER.Alto)) then
      Result := true
   else
      Result := false;
end;
